...
def balanced_retrieve(buffer, num_retrieve, excl_labels=None, return_indices=False):
    avail_labels = list(buffer.labeldict.keys())
    if excl_labels is not None:
        avail_labels = np.array(list(set(avail_labels) - set(excl_labels)))
    if len(avail_labels) > 0:
        num_instances = num_retrieve // 10
        random_labels = np.random.choice(avail_labels, 10, replace=False)
        indices = []
        for label in random_labels:
            tmp_instances = min(num_instances, len(buffer.labeldict[label]))
            if tmp_instances:
                tmp_indices = np.random.choice(buffer.labeldict[label], tmp_instances, replace=False)
